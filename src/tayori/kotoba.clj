(ns tayori.kotoba
  ...
  itself is already proven backend-agnostic (test/tayori/store_contract_test)."
  (:require [clojure.string :as str]
            [langchain.kotoba-db :as kdb]
            [tayori.cacao :as cacao]
...
(defn jvm-http-fn
  "host-caps :http-fn backed by the JDK HTTP client (no dependency). Also the
  default :http-fn for tayori.channel.email/slack/whatsapp/docport.git when a
  JVM host doesn't supply its own."
...
(defn kotoba-store
  "A `store/DatomicStore` backed by a kotoba-server pod (e.g. kotobase.net).
   Auth options (pick one):
     ...
     :identity         a tayori.cacao identity ({:private-key :public-key :did})
